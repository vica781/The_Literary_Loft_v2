...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.user.is_authenticated:
                ...
                # address if 'set_default_info' is checked
                if set_default_info:
                    try:
            ...
            for book_id, item_data in bag.items():
                try:
                    ...
                    messages.error(
                        request,
                        f"Book with ID {book_id} was not found indatabase."
                        )
                    order.delete()
                    return redirect('books:view_cart')
        ...
        if request.user.is_authenticated:
            ...
            try:
                ...
                try:
                    ...
                    initial_data = {
                        'full_name':
                            delivery_info.get('full_name') or
                            profile.default_full_name or
                            request.user.get_full_name(),
                        'email': request.user.email,
                        'phone_number':
                            delivery_info.get('phone_number') or
                            profile.default_phone_number or '',
                        'country':
                            delivery_info.get('country') or
                            profile.default_country or '',
                        'postcode':
                            delivery_info.get('postcode') or
                            profile.default_postcode or '',
                        'town_or_city':
                            delivery_info.get('town_or_city') or
                            profile.default_town_or_city or '',
                        'street_address1':
                            delivery_info.get('street_address1') or
                            profile.default_street_address1 or '',
                        'street_address2':
                            delivery_info.get('street_address2') or
                            profile.default_street_address2 or '',
                        'county':
                            delivery_info.get('county') or
                    ...
                    if delivery_info:
                        form = OrderForm(initial={
                            ...
                            'email': request.user.email,
                            'phone_number':
                                delivery_info.get('phone_number', ''),
                            'country': delivery_info.get('country', ''),
                            'postcode': delivery_info.get('postcode', ''),
                            'town_or_city':
                                delivery_info.get('town_or_city', ''),
                            'street_address1':
                                delivery_info.get('street_address1', ''),
                            'street_address2':
                                delivery_info.get('street_address2', ''),
    ...
    for book_id, item in bag.items():
        try:
            ...
            messages.error(
                request,
                f"A book in your bag was not found in our database."
                )
            return redirect('books:view_cart')
...
def order_history(request):
    ...
    if request.user.is_authenticated:
        ...
        orders = Order.objects.filter(
            ...
            ) if guest_order_number else Order.objects.none()
...
def clear_saved_info(request):
    if 'delivery_info' in request.session:
        ...
        messages.success(
            request,
            'Temporarily saved delivery information has been cleared'
            )
    else:
...
def guest_order_lookup(request):
    if request.method == 'POST':
        ...
        try:
            ...
            return render(
                request,
                'checkout/guest_order_detail.html',
                {'order': order}
                )
